...
def main(model, dataset, train_pairs, qrels_train, valid_run, qrels_valid, model_out_dir):
    ...
    for epoch in range(MAX_EPOCH):
        ...
        if top_valid_score is None or valid_score > top_valid_score:
            ...
            model.save(os.path.join(model_out_dir, 'weights_wl.p'))
...
def train_iteration(model, optimizer, dataset, train_pairs, qrels):
    ...
    with tqdm('training', total=BATCH_SIZE * BATCHES_PER_EPOCH, ncols=80, desc='train') as pbar:
        for record in data.iter_train_pairs_with_labels(model, dataset, train_pairs, qrels, GRAD_ACC_SIZE):
            ...
            scores =  scores[:, true_id].unsqueeze(dim=-1)
            relevance = record['extra_labels'] 
            loss = loss_functions.pointwise_cross_entropy(relevance,scores)
...
def main_cli():
    parser = argparse.ArgumentParser('T5 with_reranking_loss model training and validation')
    parser.add_argument('--model', choices=MODEL_MAP.keys(), default='T5_with_reranking_loss')
